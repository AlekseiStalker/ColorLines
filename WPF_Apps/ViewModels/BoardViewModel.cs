...
namespace BallsAndLines.ViewModels
{
    class BoardViewModel : BaseViewModel
    {
        ...
        { 
            ...
            {
                _cells.Add(new ObservableCollection<CellViewModel>());

                for (int col = 0; col < Settings.BoardSize; col++)
                {
                    _cells[row].Add(new CellViewModel(_gameBoard[row][col])); 
                }
            }
        } 
        ...
        public void DropRandomBalls() => GameController.CreateOnBoardNewRandomBalls();
